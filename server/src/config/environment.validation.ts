...
export function validateEnvironment(environment: Environment) {
  ...
  if (smtpConfigured) {
    ['SMTP_USER', 'SMTP_PASSWORD', 'MAIL_FROM', 'LEAD_NOTIFICATION_TO'].forEach(
      (key) => required(environment, errors, key),
    );
  }
  ...
  const storageConfigured = storageKeys.some((key) =>
...
function required(environment: Environment, errors: string[], key: string) {
  if (!environment[key]?.trim()) {
    errors.push(`${key} is required`);
  }
}

function validateUrl(environment: Environment, errors: string[], key: string) {
